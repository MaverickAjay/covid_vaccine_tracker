...
for i in range(37):
    n = states['states'][i]['state_id']
    driver.get(f"https://cdn-api.co-vin.in/api/v2/admin/location/districts/{n}")
    district_codes += '\n' + driver.find_element_by_xpath("/html/body/pre").text

pprint(district_codes)
with open("district_codes.text","w") as f:
    f.write(district_codes)
